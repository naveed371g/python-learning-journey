...
class StockProfitLossAnalyzer:
    ...
    def load_config(self):
        ...
        try:
            ...
            print(f"✅ Loaded {len(self.portfolio)} stocks from {self.yaml_file}")
    ...
    def calculate_daily_profit_loss(self):
        ...
        for stock in self.portfolio:
            ...
            for date, row in hist.iterrows():
                ...
                daily_pl_percent = ((current_price - purchase_price) / purchase_price) * 100
            ...
            print(f"   ✅ {symbol}: ${daily_pnl[-1]['profit_loss']:,.2f} ({daily_pnl[-1]['profit_loss_percent']:.2f}%)")
    ...
    def calculate_weekly_change(self):
        ...
        for symbol, data in self.profit_loss_data.items():
            ...
            weekly_change = current_day_data['profit_loss'] - monday_data['profit_loss']
            weekly_percent_change = current_day_data['profit_loss_percent'] - monday_data['profit_loss_percent']
            ...
            print(f"   ✅ {symbol}: ${weekly_change:,.2f} ({weekly_percent_change:+.2f}%) - {week_status}")
    ...
    def generate_weekly_change_table(self):
        ...
        if current_date.weekday() >= 4:  # Friday or weekend
            ...
            headers = ['Symbol', 'Monday', current_date.strftime('%A'), 'Weekly P/L', 'Weekly %']
    ...
    def generate_daily_profit_loss_table(self):
        ...
        for symbol, data in self.profit_loss_data.items():
            ...
            if len(data['daily_pnl']) >= 2:
                ...
                daily_change = latest_trading_day['price'] - previous_day['price']
                daily_change_percent = (daily_change / previous_day['price']) * 100 if previous_day['price'] > 0 else 0
                ...
                color_value_change = GREEN if (latest_trading_day['value'] - previous_day['value']) >= 0 else RED
                color_pl_change = GREEN if (latest_trading_day['profit_loss'] - previous_day['profit_loss']) >= 0 else RED
                ...
                headers = ['Period', 'Date', 'Shares', 'Price', 'Value', 'Total P/L', '% Change']
                print(tabulate(comparison_data, headers=headers, tablefmt='grid'))
                
                # Accumulate totals
                total_daily_price_change += daily_change
                total_daily_value_change += (latest_trading_day['value'] - previous_day['value'])
                total_daily_pl_change += (latest_trading_day['profit_loss'] - previous_day['profit_loss'])
            else:
        ...
        total_data = [
            ['Total Price Change', f"{total_color}{total_emoji} ${total_daily_price_change:.2f}{RESET}"],
            ['Total Value Change', f"{total_color}{total_emoji} ${total_daily_value_change:,.2f}{RESET}"],
            ['Total P/L Change', f"{total_pl_color}{total_emoji} ${total_daily_pl_change:,.2f}{RESET}"]
        ]
        
        print(tabulate(total_data, headers=['Metric', 'Value'], tablefmt='grid'))
    ...
    def plot_profit_loss_chart(self):
        ...
        ax1.set_title('Daily Profit/Loss by Stock', fontsize=14, fontweight='bold')
        ...
        if self.profit_loss_data:
            ...
            dates = [day['date'] for day in self.profit_loss_data[first_symbol]['daily_pnl']]
        ...
        if dates and portfolio_values:
            ax2.plot(dates, portfolio_values, linewidth=3, color='darkblue', label='Total Portfolio')
            ax2.fill_between(dates, portfolio_values, alpha=0.3, color='darkblue')
        ...
        plt.tight_layout()
        plt.savefig('stock_profit_loss_chart.png', dpi=150, bbox_inches='tight')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze stock portfolio profit/loss from YAML file')
    parser.add_argument('--yaml', default='stocks.yaml', help='Path to YAML configuration file')
    ...
    analyzer.run_analysis(show_chart=not args.no_chart, export_csv=not args.no_csv)
